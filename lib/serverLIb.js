...
const redirectToLoginIfLoggedOut=(req,res)=>{
  if(req.urlIsOneOf(['/home','/']) && !req.cookies.userName) res.redirect('/login');
}

const redirectToHomeIfLoggedin=(req,res)=>{
  if(req.urlIsOneOf(['/','/login']) && req.cookies.userName) res.redirect('/home');
}
...
const serveLogin = function (req, res) {
  ...
  if(req.cookies.message){
    res.setHeader('Set-Cookie', [`message="";Max-Age=5`, `sessionid=0;Max-Age=5`]);
...
const createItem = function (req, res) {
  ...
  let todoItem = {text: text,id: todoId,isDone:undefined}
