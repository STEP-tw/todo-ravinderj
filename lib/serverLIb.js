...
const getTodoView = (title,description) =>{
  let viewContent = fs.readFileSync("./templates/todoView.html","utf8");
  viewContent = viewContent.replace("${title}",title);
  viewContent = viewContent.replace("${description}",description);
  return viewContent;
}
...
const sendTodoView = (req,res)=>{
  ...
  let todoId = req.body.todoId;
  let todo = allusers[user].todos[todoId];
  let title = todo.title;
  let description = todo.description;
  let viewContent = getTodoView(title,description);
